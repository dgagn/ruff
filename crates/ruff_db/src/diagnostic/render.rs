...
impl std::fmt::Display for DisplayDiagnostic<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        let renderable = resolved.renderable(self.config.context);
...
struct Resolved<'a> {
    id: String,
    diagnostics: Vec<ResolvedDiag<'a>>,
}
...
impl<'a> Resolved<'a> {
    ...
    fn new(resolver: &FileResolver<'a>, diag: &'a Diagnostic) -> Resolved<'a> {
        ...
        diagnostics.push(ResolvedDiag::from_diagnostic(resolver, diag));
        for sub in &diag.inner.subs {
            diagnostics.push(ResolvedDiag::from_sub_diagnostic(resolver, sub));
        }
    ...
    fn renderable(&self, context: usize) -> Renderable<'_> {
        Renderable {
            id: &self.id,
            diagnostics: self
                .diagnostics
                .iter()
                .map(|diag| diag.renderable(context))
                .collect(),
        }
    }
}
...
/// both.)
#[derive(Debug)]
struct ResolvedDiag<'a> {
    severity: Severity,
    message: String,
    annotations: Vec<ResolvedAnnotation<'a>>,
}

impl<'a> ResolvedDiag<'a> {
    /// Resolve a single diagnostic.
    fn from_diagnostic(resolver: &FileResolver<'a>, diag: &'a Diagnostic) -> ResolvedDiag<'a> {
    ...
    /// for each snippet.
    fn renderable<'r>(&'r self, context: usize) -> RenderableDiag<'r> {
...
struct Renderable<'r> {
    ...
    #[allow(dead_code)]
    id: &'r str,
    diagnostics: Vec<RenderableDiag<'r>>,
}

/// A single diagnostic amenable to rendering.
#[derive(Debug)]
struct RenderableDiag<'r> {
...
impl<'r> RenderableAnnotation<'r> {
    ...
    fn to_annotate(&self) -> AnnotateAnnotation<'_> {
        ...
        // a error (red) and coloring for secondary annotations that
...
/*
/// A representation of a code snippet's range in terms of line numbers.
///
/// Specifically, this includes any surrounding "contextual" lines that
/// are added beyond what is implied by annotation spans.
struct SnippetWithContext {
    line_start: OneIndexed,
    line_end: OneIndexed,
}

impl SnippetWithContext {
    /// Creates a new line range for the given snippet and context window size
    /// (in lines).
    ///
    /// The given `line_start` and `line_end` should be the starting and ending
    /// line numbers as implied by the start of the earliest annotation and the
    /// end of the latest annotation.
    fn new(
...
mod tests {
    ...
    use crate::system::{DbWithTestSystem, SystemVirtualPath};
...
";
    ...
    impl TestEnvironment {
        ...
        fn add(&mut self, path: &str, contents: &str) {
            let path = SystemVirtualPath::new(path);
            self.db.write_virtual_file(path, contents);
        }
        ...
        fn span(&self, path: &str, line_offset_start: &str, line_offset_end: &str) -> Span {
            let path = SystemVirtualPath::new(path);
            let file = self.db.files().virtual_file(&self.db, path).file();
        ...
        fn render(&self, diag: &mut Diagnostic) -> String {
            let mut buf = vec![];
            diag.print(&self.config, &self.db, &mut buf).unwrap();
